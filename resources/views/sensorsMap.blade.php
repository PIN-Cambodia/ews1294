        ...
        @foreach($sensors24hrs as $sensor24)
          @if($sensor['sensor_id'] == $sensor24->sensor_id)
                ...
                var feature = new OpenLayers.Feature.Vector(
                    ...
                    {description:'<b>{{trans('sensors.alert_header')}}</b><br><b>Show Report of Sensor ID: {{$sensor24->sensor_id}}</b><br><p><a href ="/sensorsLog20?sensor_id={{$sensor24->sensor_id }}"><b>{{ trans('sensors.sensorlog24') }}</b> </a><br><a href ="/sensorsLog1thReadingOf30days?sensor_id={{$sensor24->sensor_id }}"><b>{{ trans('sensors.sensorlog1threadingOf30days') }}</b> </a>'} ,
                    {
                      externalGraphic: imgSensor, graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25}
                    );
                vectorLayer.addFeatures(feature);
