    ...
    public function createChart()
    {
        ...
        if($graph_type==2)
        {
            ...
            $sensorlogs = DB::table('sensorlogs')
                ->select(DB::raw("id, date_format(date(date_sub(timestamp,interval 0 hour)),GET_FORMAT(DATE,'ISO')) as time, concat(stream_height,'cm')"))
        ...
        else
        {
            ...
            $sensorlogs = DB::table('sensorlogs')
                ->select (DB::raw("id,date_format(timestamp,'%H:%i') as time,concat(stream_height,'cm')"))
                ->where('sensor_id','=',$sensor_id)
